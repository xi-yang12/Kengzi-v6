...
for i = index
    ...
    valDOther = gmmEval(gisData.PRE.data_ext(i, gisData.ModelParam.dOther), gisData.model(2).GMM);
    if sum(valDOther>0)
        warning('Exception Value! -- other_min_dist\n');
    end
    
    valDSelf = gmmEval(gisData.PRE.data_ext(i, gisData.ModelParam.dSelf), gisData.model(3).GMM);
    if sum(valDSelf>0)
    ...
    valPDist = gmmEval(gisData.PRE.data_ext(i, gisData.ModelParam.distP), gisData.model(4).GMM);
    if sum(valPDist>0)
        warning('Exception Value! -- parent distance\n');
    end
    
    valDistPP = gmmEval(gisData.PRE.data_ext(i, gisData.ModelParam.distPP), gisData.model(5).GMM);
    if sum(valDistPP>0)
        warning('Exception Value! -- parent parent distance\n');
    end
    
    c_prob(i) = valAttri + valDOther + valDSelf + valFsq + valPDist + valDistPP;
end
